...
public class FacebookSignUp {
	...
	public Response create(@QueryParam("organizationid")long organizationid,@QueryParam("name")String name,@Context UriInfo uriInfo){
		...
		List<?> usernameCheck = orgPersonService.getLoginCheck(facebookProfile.getEmail(),organizationid);
		if(usernameCheck.size()==0) 
		{   int orgpersonid =0;
			List<Object[]> userinOrg=(List<Object[]>) orgPersonService.loginCheckForOrganization(facebookProfile.getEmail(),organizationid);
            if(userinOrg.isEmpty()){        		
		Orgperson orgPerson = new Orgperson();
		Contactinfo contactinfo = new Contactinfo();			
		Email email = new Email();
		email.setUserid(facebookProfile.getEmail()); 
		...
		login.setUserid(facebookProfile.getEmail()); 
		login.setPassword(facebookProfile.getEmail()+facebookProfile.getFirstName());
		login.setLoginstatus('A');  

		Person person = new Person();  
		person.setFirstName(facebookProfile.getFirstName()); 
		person.setLastName(facebookProfile.getLastName());			 
		...
		orgpersonid = orgPersonService.save(orgPerson);
		  if(orgpersonid!=0){			
		    ...
		    alert.put("errormessage", "The username already exists, Now you can also use facebook for login to your account");
	...
	public Response getUser(@QueryParam("name")String name){
		...
		String emailadd=facebook.userOperations().getUserProfile().getEmail();	
