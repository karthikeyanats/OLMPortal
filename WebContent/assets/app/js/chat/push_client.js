...
//});
	...
	function enrolleduserlist(){
		...
		var getCourseUser= Ajax.get(courseData.getContextPath()+"/rest/conduct/participants/"+livesessionid,undefined,"json","application/json");
		
		renderTemplate("#enrolleduserinfotmpl",getCourseUser,"#contentdisplay");
	...
	var templates = {
			participants:function(){
				return "{{#each .}}<li class='particpantname' orgpersonid='{{orgpersonid}}'>" +
				"{{firstName}}</li>{{/each}}";
			},
	};
function view(){
			...
			function participants(){
				...
				renderTemplate("#enrolleduserinfotmpl",participantsList,"#contentdisplay");
			}
	
};*/
	
/*function enrolleduserlist(data){
	var data={};
	var courseid=$('[name=courseid]').val();
	var courseid=$(this).attr('courseid');
	data.courseid=courseid;
	var getCourseUser=ajaxhelperwithdata("enrolleduserlist",data);
...
//	console.log("userame");
	...
	populateFriendList();	
//	var loginUserDetails = "Welcome "+$("#firstName").val()+" <br>  <a href='#' id='logout'>Logout</a><br><br>";
			  ...
			  if(response ==""){
				  ...
				  for(var i=0;i<users.length;i++){
					  if(users[i] != $("#userName").val()){
						 $(".onlineUsersUL").append("<li><img class='onlineGreen' src='/ecloudbaseweb/assets/app/images/chat/onlineGreen.png'><a style='float:left;' href='#'>"+users[i]+"</a></li>");
...
//	self.categorywiseCoursesData(categoryidobj);
	var categorycourses=ajaxhelperwithdata("loadCategoryCourses",categoryidobj);
	renderTemplate("#enrolleduserinfotmpl",categorycourses,"#contentdisplay");
