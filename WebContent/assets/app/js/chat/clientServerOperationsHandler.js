...
function ClientServerOperationsHandler(ch, m){
	...
	ClientServerOperationsHandler.prototype.createConnection = function(){
		...
		try{
			var params = escape("channel") + "=" + escape(this.channel) +"&"+ escape(userName)+ "=" + escape(userName);
			var url = "/ecloudbaseweb/loginServlet?" + params;
...
function onMessageReceive(model){
	...
	switch(messages[0]){
		...
		break;
		case NEWUSER :
					  $(".onlineUsersUL").append("<li><img class='onlineGreen' src=''><a style='float:left;' href='#'>"+messages[1]+"</a></li>");
