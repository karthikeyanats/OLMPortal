$(document).ready(function(){
	$("#userbrowsecourse").addClass('hover').addClass('active');
	browse().init();
	browse().pageevents();
});	
...
var browse = (function () {
	...
	var controller = {	
			...
			usercourse:function(categoryid){
				return Ajax.get(courseData.getContextPath()+"/rest/browseu?categoryid="+categoryid+"",undefined,"json","application/json");
			},
			wishlist:function(courseid){
				return Ajax.post(courseData.getContextPath()+"/rest/browseu/wishlist?courseid="+courseid+"",undefined,"json","application/json");
			}
	}; 
	var model = {
			board:function(){
			...
			usercourse:function(categoryid){
				return controller.usercourse(categoryid);
			},
			wishlist:function(courseid){
				return controller.wishlist(courseid);
			},
	...
	var view = {	
			...
			courseHeader:function(){
				...
				var formatedList=formater.course(model.course(objects.category(categoryid)),model.usercourse(categoryid));
			...
			course:function(list){
				...
				_.each(courseid,function(courseids){
					var courseidlist =courseData.Schedulelist(courseids);
				...
				$('.individualcourserow').hover(function(){
					...
					case "N":
						...
						$('.newwishlist').off().click(function(e){
							var status=model.wishlist($(this).attr('courseid'));
							if(status=="OK"){
				...
				$('.startlearninggridbtn').click(function(){
					var obj=objects.startlearning($(this).attr('courseenrollmentid'),$(this).attr('courseenrollmentstatus'), $(this).attr('courseid'),$(this).attr('price'),$(this).attr('priceid'));
	...
	var formater={
			...
			courseformater:function(list){
				...
				_.each(coursevalues,function(courseid){
					...
					course=_.pick(courseArray[0],'courseid','coursetitle','price','firstName','lastName','courselogo','courserating','priceid');
	...
	var helper={
			startlearning:function(courseidobj){
				...
				case "W":
					...
					$('[name=courseenrollmentstatus]').val(courseidobj.courseenrollmentstatus);
					$('[name=price]').val(courseidobj.price);
					$('[name=priceid]').val(courseidobj.priceid);
				...
				case "P":
					$('#subscribedmodal').modal('show');
					$('.enrolstatus').text("Your subsription for the course is waiting for approval from the author. Please wait for a while.");
					break;
				case "N":
					$('[name=courseid]').val(courseidobj.courseid);
					$('[name=courseenrollmentid]').val(courseidobj.courseenrollmentid);
