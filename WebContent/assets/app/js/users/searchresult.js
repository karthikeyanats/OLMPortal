...
view={
		results:function(searchresult){
			...
			if($('.orgstatusid').val()=='admin'){	
				$('.authorsearchbox').hide();
				$('.managediv').attr("style","display:;");
			}else{
				$('.startlearndiv').attr("style","display:;");
			}
			...
			$('.managecourse').click(function()	{
				...
				$('[name=startlearninggridform]').attr('method','post');
				$('[name=startlearninggridform]').attr('action','managecourse');
...
helper={
		...
		prices:function(searchList){
			...
			var above5k=_.difference(allprices, (_.union(freecourses, onttothousand,oneto5k)));*/
		...
		redirectPage:function(courseid){
			...
			if(enrolledValue){
				...
				case "W":
					...
					$('[name=courseenrollmentid]').val(enrolledValue[0].courseenrollmentid);
					$('[name=startlearninggridform]').attr('action','previewuser');
					$('[name=startlearninggridform]').submit();	
					break;
				case "B":
						$('#messagemodal').modal('show');
						$('.mesdesc').text("You have been blocked from the course. Please contact Administrator");
						break;
				case "P":
					$('#messagemodal').modal('show');
					$('.mesdesc').text("Your subscription for the course is waiting for approval. Please wait");
					break;					
				}
			}else{
				$('[name=courseid]').val(courseid);
